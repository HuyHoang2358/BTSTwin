<template>
  ...
  >
    <template #title>
      <a-row jutify="center">
        <a-col :span="14">
          <h1 class="text-3xl mb-1">Bản đồ nền vệ tinh</h1>
        </a-col>
        <a-col :span="10">
          <div class="flex flex-row gap-x-2.5">
            ...
            <a-button
              ...
              id="add-field"
              v-if="checkPermission(`${SERVICE_LAYER_RASTER}${ACTION_STORE}`)"
            >
              {{ $t('add') }}
            </a-button>
          </div>
      ...
      <template v-if="column.dataIndex === 'action'">
        <div class="flex flex-row items-center justify-end gap-x-4">
          <a-button
            ...
            :icon="h(IconEdit)"
            :id="`edit-${record.id}`"
            v-if="checkPermission(`${SERVICE_LAYER_RASTER}${ACTION_UPDATE}`)"
          />
        </div>
      </template>
    </template>
  </a-table>
  <ModalHandleBaseLayer
    :close="() => (open = false)"
    :open="open"
...
import { useBaseLayers } from '@/services/hooks/useLayer';
import type { Layer } from '@/services/apis/layer';
import type { ColumnType } from '@/utils/types';
import ModalHandleBaseLayer from '@/components/admin/ModalHandleBaseLayer.vue';
import { checkPermission } from '@/utils/helpers';
import { ACTION_STORE, ACTION_UPDATE, SERVICE_LAYER_RASTER } from '@/utils/IAM_constants';
...
const { data, isLoading } = useBaseLayers({
